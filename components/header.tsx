...
export function Header({ account }: { account: HeaderAccount }) {
  ...
  return (
    <header className='sticky top-0 z-100 border-b border-line bg-oil'>
      ...
      {open ?
        <nav id='mobile-nav' className='grid gap-1 border-t border-line bg-asphalt px-5 py-4 md:hidden'>
          {nav.map(item => (
            <Link
              key={item.href}
              href={item.href}
          ...
          {account.enabled ?
            ...
            : <a href='/api/auth/shopify/login?returnTo=/cuenta' className='display py-3 text-5xl text-cream'>
                Entrar
              </a>
